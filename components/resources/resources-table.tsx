...
import { Mail, Clock, TrendingUp, TrendingDown, MoreHorizontal, AlertTriangle, User, Building, MapPin, CreditCard } from 'lucide-react';
...
export function ResourcesTable({ resources }: ResourcesTableProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'on_bench': return 'bg-orange-50 text-orange-700 border-orange-200';
  ...
  const getEmployeeTypeColor = (type: string) => {
    switch (type) {
      case 'fulltime': return 'bg-blue-50 text-blue-700 border-blue-200';
      case 'consultant': return 'bg-purple-50 text-purple-700 border-purple-200';
      case 'intern': return 'bg-green-50 text-green-700 border-green-200';
  ...
  const getBillingStatusColor = (status: string) => {
    switch (status) {
      case 'billable': return 'bg-emerald-50 text-emerald-700 border-emerald-200';
      case 'non_billable': return 'bg-red-50 text-red-700 border-red-200';
      case 'overhead': return 'bg-yellow-50 text-yellow-700 border-yellow-200';
      default: return 'bg-gray-50 text-gray-700 border-gray-200';
    }
  };
      ...
      <Table>
        ...
        <TableBody>
          {sortedResources.map((employee) => {
            const utilizationColor = getUtilizationColor(employee.current_utilization, employee.utilization_target);
            const utilizationStatus = getUtilizationStatus(employee.current_utilization, employee.utilization_target);
              ...
              <TableRow key={employee.id} className="hover:bg-gray-50">
                <TableCell>
                  <div>
                    ...
                    <div className="text-sm text-slate-600 font-mono">{employee.employee_id}</div>
                ...
                <TableCell>
                  <div className="space-y-2">
                    <Badge className={getEmployeeTypeColor(employee.employee_type)}>
                      ...
                      {employee.employee_type === 'fulltime' ? 'Full-time' : 
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {employee.status === 'on_bench' && employee.bench_time > 30 && (
                ...
                <TableCell>
                  <div className="space-y-2">
                    ...
                    <Badge className={getBillingStatusColor(employee.billing_status)}>
                ...
                <TableCell>
                  {employee.status === 'on_bench' ? (
                    <div className="flex items-center text-sm">
                      ...
                      <span className={employee.bench_time > 30 ? 'text-red-600 font-medium' : 'text-orange-600'}>

...
import { Plus, BarChart3, Download, Users, AlertTriangle, TrendingUp, Upload, User, Building, MapPin } from 'lucide-react';
...
export default function EmployeesPage() {
  ...
  const filteredEmployees = employees.filter(employee => {
    ...
    if (filters.search && !employee.name.toLowerCase().includes(filters.search.toLowerCase()) && 
        ...
        !employee.employee_id.toLowerCase().includes(filters.search.toLowerCase())) return false;
    if (filters.role !== 'all' && employee.role !== filters.role) return false;
    if (filters.employeeType !== 'all' && employee.employee_type !== filters.employeeType) return false;
    ...
    if (filters.utilization !== 'all') {
      if (filters.utilization === 'underutilized' && employee.current_utilization >= employee.utilization_target * 0.7) return false;
      if (filters.utilization === 'optimal' && (employee.current_utilization < employee.utilization_target * 0.7 || employee.current_utilization > employee.utilization_target * 1.1)) return false;
      if (filters.utilization === 'overutilized' && employee.current_utilization <= employee.utilization_target * 1.1) return false;
    }
  ...
  const benchEmployees = filteredEmployees.filter(e => e.status === 'on_bench');
  const fulltimeEmployees = filteredEmployees.filter(e => e.employee_type === 'fulltime');
  const consultantEmployees = filteredEmployees.filter(e => e.employee_type === 'consultant');
  const billableEmployees = filteredEmployees.filter(e => e.billing_status === 'billable');
  const averageUtilization = activeEmployees.length > 0 
    ? activeEmployees.reduce((sum, e) => sum + e.current_utilization, 0) / activeEmployees.length 
    : 0;
  const underutilizedCount = activeEmployees.filter(e => e.current_utilization < e.utilization_target * 0.7).length;
        ...
        <main className="flex-1 overflow-y-auto p-6 lg:p-8 space-y-8">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {(profile?.role === 'admin' || profile?.role === 'ceo' || profile?.role === 'hr') && (

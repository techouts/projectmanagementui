...
export default function DashboardPage() {
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      ...
      case 'pm': return LayoutDashboard;
  ...
  const getRoleColor = (role: string) => {
    switch (role) {
      case 'ceo': return 'from-yellow-500 to-orange-600';
      case 'pm': return 'from-blue-500 to-blue-600';
      case 'finance': return 'from-green-500 to-green-600';
      case 'hr': return 'from-purple-500 to-purple-600';
      case 'admin': return 'from-gray-500 to-gray-600';
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-4">
          <div className={`w-20 h-20 bg-gradient-to-br ${roleColor} rounded-2xl flex items-center justify-center mx-auto shadow-lg`}>
          ...
          <div>
            ...
            <Badge className={`mt-3 bg-gradient-to-r ${roleColor} text-white border-0 text-sm px-4 py-1`}>
        ...
        <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <Card key={index} className={`shadow-lg border-0 ${stat.bgColor} ${stat.borderColor} border hover:shadow-xl transition-all duration-300 group`}>
          ...
          <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
            ...
            <CardContent>
              <div className="space-y-4">
                {[
                  { action: 'Project created', item: 'Mobile App Redesign', time: '2 hours ago', type: 'success' },
                  { action: 'Team member added', item: 'Sarah Johnson', time: '4 hours ago', type: 'info' },
                  { action: 'Milestone completed', item: 'API Development', time: '1 day ago', type: 'success' },
                  { action: 'Budget updated', item: 'E-commerce Platform', time: '2 days ago', type: 'warning' }
                ].map((activity, index) => (
          ...
          <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
            ...
            <CardContent>
              <div className="grid gap-3">
                ...
                <Button variant="outline" className="w-full justify-start h-12 rounded-xl border-slate-200 hover:bg-slate-50 transition-all duration-200">
          ...
          <CardContent className="p-6">
            <div className="text-center">
              ...
              <p className="text-slate-600">
                {profile.role === 'ceo' && 'Access comprehensive business insights and strategic overview.'}
                {profile.role === 'pm' && 'Manage projects, track progress, and coordinate team activities.'}
                {profile.role === 'finance' && 'Monitor budgets, track expenses, and generate financial reports.'}
                {profile.role === 'hr' && 'Manage team resources, track utilization, and handle HR operations.'}
                {profile.role === 'admin' && 'Configure system settings and manage user access.'}
              </p>
            </div>
          </CardContent>
        </Card>
